...
- **经验严重偏斜**：约 74% 的岗位为「1 年以下」，这是猎聘按经验分档反复抓取的结果，属采集机制的必然产物而非市场真实比例。建模时用**逆频率加权 + 分层划分**处理（见「薪资预测模型」）。
...
**当前模型结果**（基于 26851 条有效薪资样本，随机森林最优）：

| 模型 | 交叉验证 R² | 测试集 R² | MAE（K/月） |
|------|-----------|----------|------------|
| 线性回归 | 0.248 | 0.244 | 6.94 |
| **随机森林（最优）** | **0.293** | **0.308** | **6.50** |
| 梯度提升 | 0.281 | 0.304 | 6.56 |
...
- **样本偏斜处理**：经验分布严重偏向「1 年以下」（约 74%，是猎聘按经验分档抓取的必然结果），训练时按经验档**逆频率加权**（`sample_weight`）缓解低经验主导，且不丢数据；`train_test_split` 按经验**分层划分**，保证评估集经验分布一致。
- **特征重要性**：用 `permutation_importance`（比树自带的 `feature_importances_` 更公平，论文里也好解释）。当前排序：**学历 0.355 > 职位方向 0.231 > 经验 0.188 > 城市 0.134 > 技能数 0.056 > 来源 0.036**。学历解释力居首，与下文统计推断的 ANOVA 效应量结论互相印证。
...
|------|---------|------|-----------|------|
| 学历 | 1280.45 | <0.001 | **0.167（大效应）** | 解释 16.7% 薪资方差，三因素中最强 |
| 经验 | 426.24 | <0.001 | 0.060（小-中） | 解释 6.0% 方差，随经验单调递增 |
| 城市 | 88.42 | <0.001 | 0.059（小-中） | 解释 5.9% 方差 |

- 三因素对薪资的影响**均统计显著**，但 η² 揭示**学历的解释力远强于城市和经验**——这与随机森林 permutation 特征重要性（学历 0.355 居首）**两个独立方法互相印证**，是分析章的核心论点。
